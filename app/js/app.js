...
createApp({
    setup() {
        ...
        const kasirSearch = ref(''); // pencarian di layar Kasir (terpisah dari pencarian di Kelola Produk)
        ...
        const filteredKasirProducts = computed(() => {
            const q = kasirSearch.value.trim().toLowerCase();
            if (!q) return products.value;
            return products.value.filter((p) => {
                const categoryName = categoryNameById(p.category_id) || '';
        ...
        // ========================
        // PENGATURAN (bisa dikunci PIN — supaya cuma pemilik/penjual app yang pegang penuh)
        // ========================
        ...
        const openSettings = () => {
            ...
            newPinInput.value = '';
            // Kalau belum pernah diset PIN, Pengaturan terbuka seperti biasa (supaya bisa
            // dikonfigurasi pertama kali). Setelah PIN diset & disimpan, baru terkunci.
            settingsUnlocked.value = !config.value.admin_pin_hash;
            showSettingsModal.value = true;
        };

        const unlockSettings = async () => {
            if (!settingsPinInput.value) return;
            const hash = await sha256Hex(settingsPinInput.value);
            if (hash === config.value.admin_pin_hash) {
        ...
        const saveSettings = async () => {
            const updated = { ...config.value, ...settingsForm.value };
            if (newPinInput.value) {
                updated.admin_pin_hash = await sha256Hex(newPinInput.value);
            }
        ...
        const removeSettingsPin = () => {
            newPinInput.value = '';
            config.value = { ...config.value, admin_pin_hash: '' };
            saveConfig(config.value);
        ...
        // ========================
        // AUTO-UPDATE (service worker versi baru dari GitHub Pages)
        // ========================
        ...
        // Reload otomatis begitu aman dilakukan (keranjang kosong, tidak ada modal terbuka)
        // supaya transaksi yang sedang berjalan tidak pernah hilang gara-gara update.
        const applyUpdateIfSafe = () => {
            if (!updateReloadPending) return;
            if (cart.value.length === 0 && !isAnyModalOpen()) {
        ...
        watch(
            [cart, showReceiptModal, showProductModal, showCategoryModal, showSupplierModal, showSettingsModal],
            applyUpdateIfSafe
        );
        ...
        onMounted(async () => {
            ...
            if ('serviceWorker' in navigator) {
                ...
                // Service worker (lihat service-worker.js) selalu skipWaiting + clients.claim,
                // jadi begitu ada versi baru di GitHub Pages, browser otomatis pasang & aktifkan
                // di background -> event ini yang menandai "versi baru sudah mengambil alih".
